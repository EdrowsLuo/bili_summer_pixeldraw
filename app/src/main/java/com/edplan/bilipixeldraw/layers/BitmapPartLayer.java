...
public class BitmapPartLayer extends Layer
{
	...
	public Matrix bitmapOverlayMatrix;
	public Point bitmapOverlayOffset;
	public Bitmap bitmapOverlay;
	public boolean bitmapOverlayVisible=true;
	public boolean bitmapOverlayCheckerEnable=false;
	public Bitmap bitmapOverlayChecker;
	public Paint bitmapOverlayPaint;
	public Paint checkPointPaint;
	...
	public BitmapPartLayer(){
		...
		checkPointPaint=new Paint();
		checkPointPaint.setStrokeWidth(StaticM.dip2px(0.5f));
		checkPointPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
		
		bitmapOverlayPaint=new Paint();
		bitmapOverlayPaint.setAntiAlias(false);
		bitmapOverlayPaint.setAlpha(255);
	...
	public void setBitmapOverlayVisible(boolean bitmapOverlayVisible)
	{
		this.bitmapOverlayVisible = bitmapOverlayVisible;
	}

	public boolean isBitmapOverlayVisible()
	{
		return bitmapOverlayVisible;
	}
	
	public void setBitmapOverlay(Bitmap b,Point p){
		bitmapOverlayMatrix=new Matrix();
		bitmapOverlay=b;
		bitmapOverlayOffset=p;
	}
	...
	public void setZoomTimes(float zoom){
		...
		if(zoom>StaticM.dip2px(60f))zoom=StaticM.dip2px(60f);
		zoomTimes=zoom;
	}
	...
	private void updateMatrix(){
		...
		if(bitmapOverlayMatrix!=null){
			bitmapOverlayMatrix.setTranslate(0.00f+saveCanvasRect.centerX()-centerPoint.x+bitmapOverlayOffset.x,0.00f+saveCanvasRect.centerY()-centerPoint.y+bitmapOverlayOffset.y);
			bitmapOverlayMatrix.postScale(zoomTimes,zoomTimes,saveCanvasRect.centerX(),saveCanvasRect.centerY());
		}
	...
	public void onTouch(MotionEvent event)
	{
		...
		switch(event.getAction()){
			...
			case MotionEvent.ACTION_MOVE:
				...
				float x=downBitmapCenter.x+(downPoint.x-event.getX())/getWidthPerPixel();
				float y=downBitmapCenter.y+(downPoint.y-event.getY())/getWidthPerPixel();
				if(x<0||x>bitmap.getWidth()){
					downPoint.x=event.getX();
					downBitmapCenter.x=centerPoint.x;
					x=downBitmapCenter.x;
				}
				if(y<0||y>bitmap.getHeight()){
					downPoint.y=event.getY();
					downBitmapCenter.y=centerPoint.y;
					y=downBitmapCenter.y;
				}
				PointF moveV=new PointF(downPoint.x-event.getX(),downPoint.y-event.getY());
				//x=downBitmapCenter.x+(downPoint.x-event.getX())/getWidthPerPixel();
				//y=downBitmapCenter.y+(downPoint.y-event.getY())/getWidthPerPixel();
	...
	public void draw(Canvas canvas)
	{
		...
		if(bitmapOverlay!=null){
			if(bitmapOverlayVisible){
				canvas.drawBitmap(bitmapOverlay,bitmapOverlayMatrix,bitmapOverlayPaint);
				//if(bitmapOverlayCheckerEnable)drawBitmapOverlayChecked(canvas);
			}
		}
	...
	public void drawDatas(Canvas canvas){
		if(selectedPoint!=null){
			...
			if(enablePositionText)canvas.drawText(getDataTxt(),rb.x+10,rb.y+10,textPaint);
		}
	}
	...
	public String getDataTxt(){
		return "("+selectedPoint.x+","+selectedPoint.y+")";
	}
	
	public void drawBitmapOverlayChecked(Canvas canvas){
		int color;
		int xr,yr;
		RectF r;
		for(int x=0;x<bitmapOverlay.getWidth();x++){
			for(int y=0;y<bitmapOverlay.getHeight();y++){
				color=bitmapOverlay.getPixel(x,y);
				if(color!=0x00000000){
					xr=x+bitmapOverlayOffset.x;
					yr=y+bitmapOverlayOffset.y;
					if(inBitmap(xr,yr)){
						if(color!=bitmap.getPixel(xr,yr)){
							r=getBitmapPixelRect(xr,yr);
							canvas.drawPoint(r.centerX(),r.centerY(),checkPointPaint);
						}
					}
	...
	public RectF getBitmapPixelRect(int x,int y){
		PointF lt=bitmapPointToCanvas(new PointF(x,y));
		PointF rb=bitmapPointToCanvas(new PointF(x+1,y+1));
